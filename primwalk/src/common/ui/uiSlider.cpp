  ...
  void UISlider::onRender(UIRenderSystem& renderer)
  {
    ...
    if (m_Hovered) {
      renderer.drawRect(getAbsolutePosition() +
        glm::vec2((normCurrentVal / newMaxVal) * m_Width - m_SliderRadius, m_TrackHeight / 2 - m_SliderRadius), m_SliderRadius * 2, m_SliderRadius * 2, m_SliderColor, m_SliderRadius + 1);
    }
  }
