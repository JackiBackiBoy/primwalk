...
namespace pw {
	...
	SubView::~SubView() {
		...
		vkDestroyRenderPass(((GraphicsDevice_Vulkan*&)pw::GetDevice())->getDevice(), m_OffscreenPass->getVulkanRenderPass(), nullptr);
	}
	...
	void SubView::beginPass(VkCommandBuffer commandBuffer) {
		...
		glm::vec4 normColor = Color::normalize(m_BackgroundColor);
	...
	void SubView::resize(int width, int height) {
		...
		vkDeviceWaitIdle(((GraphicsDevice_Vulkan*&)pw::GetDevice())->getDevice());
