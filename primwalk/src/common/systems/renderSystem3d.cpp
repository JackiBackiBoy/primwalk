...
namespace pw {
	...
	void RenderSystem3D::onUpdate(const FrameInfo& frameInfo, ComponentManager& manager) {
		...
		drawDebugLine({ 0.0f, 0.0f, 0.0f }, { 10.0f, 1.0f, 10.0f });
	}
	...
	void RenderSystem3D::onRender(const FrameInfo& frameInfo, ComponentManager& manager) {
		...
		vkCmdBindDescriptorSets(frameInfo.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
			m_MainPipelineLayout, 1, 1, &m_Device.m_TextureDescriptorSet, 0, nullptr);
		...
		for (const auto& e : entities) {
			...
			for (const auto& mesh : model->getMeshes()) {
				...
				if (diffuseMap) {
					bool hasID = m_Device.getTextureID(diffuseMap->getImage(), &texIndex);

					if (!hasID) {
						texIndex = m_Device.addTextureID(diffuseMap->getImage());

						VkDescriptorImageInfo imageInfo{};
						imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
						imageInfo.imageView = diffuseMap->getImageView();
				...
				if (normalMap) {
					bool hasID = m_Device.getTextureID(normalMap->getImage(), &texIndex);

					if (!hasID) {

						texIndex = m_Device.addTextureID(normalMap->getImage());

						VkDescriptorImageInfo imageInfo{};
						imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
						imageInfo.imageView = normalMap->getImageView();
						imageInfo.sampler = Renderer::m_TextureSampler;
	...
	void RenderSystem3D::createDescriptorSetLayout() {
		...
		for (size_t i = 0; i < m_UniformDescriptorSets.size(); i++) {
			...
			DescriptorWriter(*m_UBOSetLayout, *device->m_BindlessDescriptorPool)
		...
		for (size_t i = 0; i < m_DebugStorageDescriptorSets.size(); i++) {
			...
			DescriptorWriter(*m_DebugSSBOSetLayout, *m_Device.m_BindlessDescriptorPool)
				.writeBuffer(0, &bufferInfo)
