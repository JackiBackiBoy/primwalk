...
namespace pw {
  ...
  void RenderSystem3D::onRender(const FrameInfo& frameInfo)
  {
    ...
    for (const auto& e : entities) {
      auto& component = ComponentManager::Get().getComponent<Renderable>(e);
      Model* model = component.model;

      if (model != nullptr) {

        ModelPushConstant push{};
        push.modelMatrix = glm::translate(push.modelMatrix, ComponentManager::Get().getComponent<Transform>(e).position);
